 ...
 */
class ShareMenu extends GestureEventListeners(PolymerElement) {
  static get template() {
    return html`
    <style>
      [hidden] {
        ...
        <dom-if if="[[_isEnabledServices(enabledServices, 'clipboard')]]">
          <template>
            <div class="social" title="Copy to clipboard">
              <paper-icon-button icon="social:clipboard" class="icon" id="clipboard" data-clipboard-text\$="[[_fullText]]">
              </paper-icon-button>
    ...
    if (!this._nativeShare || !this._secureContext) {
      ...
      for (let social of socials) {
        if (social.id !== 'clipboard')
          addListener(social, 'tap', () => this[`_${social.id}Tap`]())
      }
    }
  }
